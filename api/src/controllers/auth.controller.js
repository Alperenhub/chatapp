...
export const login = async (req, res) =>{
    ...
    try{
        ...
        if(!user) return res.status(400).json({message:"Kullanıcı adı veya parola hatalı"})
            
            const isPasswordCorrect = await bcrypt.compare(password, user.password)
            if(!isPasswordCorrect) return res.status(400).json({message:"Kullanıcı adı veya parola hatalı."});
...
export const logout = async (_,res) =>{
    ...
    res.status(200).json({message:"Başaryla çıkış yapıldı"});
};
...
export const updateProfile = async(req,res)=>{
    ...
    try{
      ...
      if(!profilePic) return res.status(400).json({message: "Profil fotoğrafı gereklidir"});
